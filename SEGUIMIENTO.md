...
| T13 | **Instalar la skill de consumo para cualquier sesión de la máquina**, no sólo las que abren este repo: un instalador en `ops/` que la engancha con un enlace simbólico en `~/.claude/skills/` — enlace y no copia, para que la fuente siga siendo una sola (el repo). Si en el destino hay algo que no es un enlace, se niega: no pisa lo de otro | 2026-08-08 | en curso (agente) |
