...
| T34 | **La pantalla es una página vertical y tiene que ser una app con menús.** Pedido de Pablo el 2026-08-15: *«tipo página vertical no sirve, tiene que ser una app con menús y el primer Dashboard el de los agentes de la consola»*. Hoy las cuatro secciones se apilan y para llegar a los proyectos hay que pasar por todo lo demás. El shell se copia del de ERP 360 (`static/index.html`): barra arriba, menú lateral en pantalla ancha y **tabbar abajo en teléfono**. Y hay una consecuencia de la regla 3 que este cambio obliga a resolver: **una vista escondida no puede esconder un error**. Con todo apilado, un «no pude leer» se veía al pasar; con pestañas, hay que navegar hasta él — o sea que nadie lo ve. Por eso la pestaña se marca cuando su sección tiene algo roto o algo en «no sé», y la frescura vive en la barra y no adentro de una vista | 2026-08-15 | **hecho** · `verificar-pantalla.js` 115 aserciones (§ 10) — el shell copiado del de ERP 360, cuatro vistas, la consola primero. **Visto en rojo** con `irA()` sin esconder las otras (3 rojas), con la marca apagada (3) y con el arranque apuntado a otra vista (2). La marca tiene aserción de **los dos lados**: sale con una consola caída aunque la pestaña esté escondida, y **no sale** con todo sano. Y mirado a 375 px y a 1000 px, que es donde salieron los dos bugs que ningún arnés vio |
| T35 | **Había dos `function pintar(d)` en `capataz.html`, y en JavaScript gana la última.** La primera —quince líneas, la del día cero, que sólo dibujaba las tarjetas de proyecto— quedó cuando se agregó la vista en vivo y hacía rato que no se ejecutaba. Dos versiones de la misma función y ninguna regla sobre cuál gana, adentro del proyecto que tiene justo eso escrito como regla 1. Estaba en `HEAD` y en todos los commits anteriores; apareció al reordenar la pantalla, porque un `Edit` no supo a cuál de las dos apuntar | 2026-08-15 | **hecho** · borrada, con el porqué escrito en su lugar. La suite quedó igual de verde: **no la vigilaba nadie**, que es exactamente lo que pasa con el código muerto |
| T32 | **Un agente que trabaja sobre `main` no aparece en «Agentes ahora».** Encontrado el 2026-08-15 con un caso de verdad: había una sesión trabajando en Finca 360 —empujando a `main` cada pocos minutos— y la vista primaria no mostraba a nadie de ese proyecto. Capataz no se equivoca: esa vista sale de GitHub y su modelo de «alguien trabajando» son **las ramas por delante de `main`** más los `en curso (fulano)` del seguimiento. Finca 360 tiene **una sola rama, `main`** —que se clasifica `principal`, no un agente— y **cero `en curso`** en su seguimiento (23 pendiente, 6 hecho, 3 diferido y **13 sin estado**). O sea que no hay nada publicado que diga que alguien trabaja. Falta decidir qué hacer: la salida honesta no es inventar un agente a partir de un commit en `main`, es **decir que ese proyecto no publica en ramas**, porque hoy la ausencia se lee como «no hay nadie» y en realidad es «no se puede saber por este camino». El taller sí lo ve, pero sólo si corre en esta máquina | 2026-08-15 | pendiente |
| T33 | **`finca360-iot` es un repositorio propio y no está vigilado.** Tiene remoto (`pablosilveira16/finca360-iot`) y una sesión trabajando ahí, y `proyectos.json` declara tres proyectos y no incluye ése: su sesión se muestra en el taller rotulada «carpeta que capataz no vigila» —que es lo correcto, no se filtra— y su tarjeta no existe. Se arregla con tres líneas en `proyectos.json`; es de Pablo porque decidir qué proyectos se vigilan no lo decide capataz | 2026-08-15 | pendiente (Pablo) |
