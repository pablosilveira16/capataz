...
| T12 | **Al llegar al umbral, la skill pregunta en vez de decidir sola.** Pedido de Pablo: cuando la decisión sea `cerrar` o `parar`, el agente le manda la pregunta con los números medidos y él contrasta contra lo que ve en su panel de uso — la medición local no descuenta la compactación, así que el que tiene el dato real es él. La respuesta corrige, no reemplaza: sin respuesta posible, vale la tabla | 2026-08-08 | en curso (agente) |
