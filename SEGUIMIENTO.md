...
| T15 | **`verificar-nube.py` da por sentado que `erp360` es privado, y los dos repositorios son públicos.** Medido el 2026-08-06 desde la Mac: `git ls-remote https://github.com/pablosilveira16/erp360` con `GIT_CONFIG_GLOBAL=/dev/null` y sin ninguna credencial contesta `443178a…`, y `capataz` igual. Por eso el § «un repositorio privado sin credencial» se pone rojo en tres aserciones: no es que la lectura falle, es que **sí se lee**. El arnés necesita un repositorio que de verdad no se alcance —uno inexistente ya lo tiene, falta uno privado— o la premisa escrita al lado del caso. Hasta que se arregle, el rojo tapa el caso que sí importa | 2026-08-06 | pendiente |
| T16 | **`verificar-credenciales.py` se pone rojo con su propio fixture, y ya está empujado.** La aserción «ningún commit de la historia contiene algo con forma de token» encuentra el `github_pat_11ABCDEFG…` **falso** de `pruebas/verificar-nube.py:326`, que es con lo que se prueba `nube.limpiar_secreto()`. No hay ninguna credencial filtrada —el literal es inventado— pero está en `270bb84`, así que **el rojo lo va a dar cualquier clon y también el CI** (→ D4, que es la corrida que falta mirar). El arreglo es que el fixture no exista como literal: armarlo concatenando, y que la aserción siga viendo rojo con un token puesto a mano | 2026-08-06 | pendiente |
| T17 | **No hay `node` en esta Mac, así que la pantalla no se verifica: 0 aserciones, y cero es falla.** `verificar-pantalla.js` es el único arnés que ejecuta el JavaScript de `capataz.html` —donde vive la regla 3, «no sé» nunca verde— y sin `node` saltea entero. Medido el 2026-08-06: la suite dio **335 aserciones y 4 rojas**, y las ~50 de la pantalla no estaban ni en el total ni en el rojo. Se arregla instalando `node`; lo que no se puede es dejar que saltee en silencio | 2026-08-06 | pendiente |
